    ...
    <>
      <div>
        <div className="container mx-auto px-10 py-10">
          {cart.length === 0 ? (
            <img src="/Images/cartimg/Untitled-1.pngص.png" className="flex items-center justify-center sm:w-[400px] xs:w-[350px] m-auto" alt="" />
          ) : (
            <div className="flex flex-wrap items-start gap-y-5 justify-center gap-x-5">
                ...
                {cart.map((product, index) => (
                  <li key={index} className="flex justify-between items-center p-4 bg-white lg:w-[1000px] md:w-[700px] sm:w-[500px] xs:w-[400px] shadow-md rounded-lg">
                    <div className="flex gap-5">
                      ...
                      <div>
                        <h2 className="sm:text-lg text-blue-400 xs:text-sm">{product.name}</h2>
                        <div className="flex items-center gap-20 pt-7">
                        <p className=" text-sm text-gray-600">قیمت محصول : {product.price} تومان</p>
                        <div className="pt-1 text-blue-500 text-sm flex items-center gap-2">
              ...
              <div className="bg-white w-[350px] h-[250px] rounded-xl shadow-lg">
                <h1 className="flex justify-center bg-blue-600 text-white rounded-t-xl h-[50px] items-center">محاسابت سبد خرید شما</h1>
                ...
                <div>
                  <Link to="/continuBuying">
                    <span className="text-white text-sm bg-blue-600 flex items-center justify-center w-[200px] h-[35px] hover:opacity-70 m-auto rounded-md mt-5 shadow-md cursor-pointer">ادامه فرایند خرید</span>
                  </Link>
                </div>
              </div>
            </div>
